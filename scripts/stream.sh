...
## NODE_TYPE set in .env file to one of: 
## "research" -- writes FLAC files to archive bucket; 
## "debug" -- streams only MPEG-DASH segments to streaming bucket;
## "hls-only" -- streams only HLS segments to streaming bucket;
## "dev-virt-s3" -- loops local .wav file to S3 bucket;
...
if [ $NODE_TYPE = "research" ]; then
  ...
  nice -n -10 ffmpeg -f alsa -ac $CHANNELS -ar $SAMPLE_RATE -thread_queue_size 1024 -i hw:$AUDIO_HW_ID -ac $CHANNELS \
    -f segment -segment_list "/audio/$NODE_NAME/hls/$timestamp/live.m3u8" -segment_list_flags +live -segment_time $SEGMENT_DURATION -segment_format mpegts \
    -ar $STREAM_RATE -ac 2 -threads 3 -acodec aac "/audio/$NODE_NAME/hls/$timestamp/live%03d.ts"
  ...
  nice -n -10 ffmpeg -re -fflags +genpts -stream_loop -1 -i "samples/haro-strait_2005.wav" \
    -f segment -segment_list "/audio/$NODE_NAME/hls/$timestamp/live.m3u8" -segment_list_flags +live -segment_time $SEGMENT_DURATION -segment_format mpegts \
    -ar $STREAM_RATE -ac $CHANNELS -threads 3 -acodec aac "/audio/$NODE_NAME/hls/$timestamp/live%03d.ts"
else
  SAMPLE_RATE=48000
  STREAM_RATE=48000
  echo "Sampling $CHANNELS channels from $AUDIO_HW_ID at $SAMPLE_RATE Hz with bitrate of 32 bits/sample..."
